...
const getFileIconComponent = (f: string) => {
  ...
  if (name.includes('walkthrough'))
    return <Icon icon={Waypoints} style={{ color: 'var(--color-text-secondary)' }} />;
  if (name.includes('task'))
    return <Icon icon={ListTodo} style={{ color: 'var(--color-text-secondary)' }} />;
  if (name.includes('implementation_plan'))
    return <Icon icon={Map} style={{ color: 'var(--color-text-secondary)' }} />;
  if (name.endsWith('.tsx') || name.endsWith('.ts') || name.endsWith('.jsx'))
    return <SiReact size={14} style={{ color: '#61DAFB' }} />;
  if (name.endsWith('.md'))
    return <Icon icon={FileText} style={{ color: 'var(--color-text-secondary)' }} />;
...
function parseAgenticLog(text: string) {
  ...
  if (contentLines.length > 0) {
    taskTitle = contentLines[0].replaceAll(/^\*\*|##?\s?|\*\*$/g, '').trim();
      ...
      {blocks.map((block, i) => {
        ...
        if (block.type === 'action') {
          return (
            ...
            >
              <Icon
                icon={block.target.includes('.') ? FileText : FolderTree}
                style={{ opacity: 0.7 }}
              />
              <span>
                <b>{block.action}</b> {block.target}
              </span>
            </Flexbox>
          );
        }
        ...
        <>
          ...
          <div className={styles.sectionBlock}>
            ...
            <Flexbox
              horizontal
              gap={12}
              style={{ flexWrap: 'wrap', marginTop: 12, marginBottom: 12 }}
            >
              {files.map((f) => (
            ...
            <Flexbox horizontal align="center" justify="space-between" style={{ marginBottom: 16 }}>
              ...
              <div className={styles.collapseAllBtn} onClick={() => setExpandAll(!expandAll)}>
                ...
                <Icon
                  icon={ChevronDown}
                  style={{
                    transform: expandAll ? 'rotate(180deg)' : 'none',
                    transition: 'all 0.2s',
                  }}
                />
              </div>
